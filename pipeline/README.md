...
```bash
# PDF 파일
python3 pipeline/ingest.py "raw/7. Harness and Skills.pdf"

# 텍스트 파일
python3 pipeline/ingest.py "raw/notes.txt"
```
...
```bash
python3 pipeline/ingest.py --text "MCP는 AI용 USB 인터페이스다..." --title "MCP 빠른 참조"
```

### 미리보기 (실제 저장 없이)

```bash
python3 pipeline/ingest.py --dry-run "raw/파일.pdf"
```
